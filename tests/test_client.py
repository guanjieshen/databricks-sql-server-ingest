...
class TestSkipUnchangedTables:
    ...
    @patch("azsql_ct.client.sync_table")
    @patch("azsql_ct.client.AzureSQLConnection")
    def test_no_skip_when_mode_full(self, MockAz, mock_sync, mock_compute):
        mock_conn = MagicMock()
        MockAz.return_value.connect.return_value = mock_conn
        mock_sync.return_value = {"ok": True}
    ...
    def test_change_check_failure_falls_back_to_sync_all(self, MockAz, mock_sync, mock_compute):
        ...
        MockAz.return_value.connect.return_value = mock_conn
        mock_sync.return_value = {"ok": True}
        mock_compute.side_effect = RuntimeError("change check failed")
...
class TestSetDatabricksTaskValues:
    ...
    def test_returns_total_when_no_dbutils(self):
        ...
        # No __main__.dbutils - simulate local run
        total = set_databricks_task_values(results)
        assert total == 42
